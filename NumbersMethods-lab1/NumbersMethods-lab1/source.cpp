...
{
    return fabs(supX(x, h));  // Так как третья производная от экспоненты - сама экспонента, то максимум будет при максимальном значении x
}
...
double searchH(double x, double h)
{
    ...
    return pow((3.0 * E) / M3(x, h), (1.0/3.0));                                       // Нахождение оптимального шага
}
